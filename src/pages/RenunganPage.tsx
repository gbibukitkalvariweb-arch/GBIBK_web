    ...
    <div className="min-h-screen bg-white pt-28 pb-20">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="mb-10">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-[#2A3338] uppercase tracking-tighter leading-none mb-4">Renungan & Buletin</h1>
        ...
        <div className="flex gap-3 mb-12 flex-wrap">
          {CATEGORIES.map((cat) => (
            <button key={cat.slug} onClick={() => setActiveCategory(cat.slug)}
              className={`px-5 py-2 rounded-full text-sm font-bold transition-all border ${activeCategory === cat.slug ? 'bg-[#A47151] text-white border-[#A47151]' : 'bg-white text-[#2A3338] border-gray-200 hover:border-[#A47151] hover:text-[#A47151]'}`}>
              {cat.label}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          <div className="flex justify-center py-32 animate-pulse font-bold text-gray-400 uppercase tracking-widest text-xs">Sedang Memuat...</div>
          ...
          <>
            {featured && (
              <div className="flex flex-col lg:flex-row gap-8 mb-16 cursor-pointer group" onClick={() => navigate(`/renungan/${featured.id}`)}>
                <div className="w-full lg:w-1/2 flex-shrink-0 overflow-hidden rounded-3xl shadow-xl aspect-video lg:aspect-auto lg:h-[400px]">
                  {featured.gambar ? (
                    <img src={getImageUrl(featured.gambar) || ''} alt={featured.judul} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                  ) : (
                ...
                <div className="w-full lg:w-1/2 flex flex-col justify-center">
                  <div className="flex flex-wrap items-center gap-3 mb-4">
                    <span className="bg-[#F4F1ED] text-[#A47151] px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest">{getCategoryLabel(activeCategory)}</span>
                    {featured.tanggal_publish && (
                      <span className="text-gray-400 text-sm">{new Date(featured.tanggal_publish).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                    )}
                  </div>
                  <h2 className="text-2xl md:text-4xl font-black text-[#2A3338] mb-3 group-hover:text-[#A47151] transition-colors leading-tight">{featured.judul}</h2>
                  <span className="text-[#A47151] font-bold text-sm group-hover:underline">Klik untuk membaca selengkapnya →</span>
                </div>
              </div>
            )}
                  ...
                  {posts.map((post) => (
                    <div key={post.id} className="group cursor-pointer" onClick={() => navigate(`/renungan/${post.id}`)}>
                      ...
                      {post.tanggal_publish && (
                        <p className="text-[10px] text-gray-400 mb-1">{new Date(post.tanggal_publish).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                      )}
                      <h4 className="text-sm font-bold text-[#2A3338] group-hover:text-[#A47151] transition-colors leading-tight line-clamp-2">{post.judul}</h4>
                    </div>
                  ))}
                </div>
              </>
